...
- A subdirectory storing intermediate files generated by **`Phenix.mtriage`**.
- Subdirectories containing auxiliary outputs from **`Phenix.emringer`**, inlcuding both result files and automatically generated plots.
- A subdirectory with auxiliary files produced during **MapQ execution**, such as per-residue Q-score text outputs.
...
**Note:** users who only intend to compute metrics using the post-processed maps already included in the dataset do not need to install the post-proceesing methods, but Phenix and MapQ are always required. 
...
| Argument | Description | 
|-------|-----|
|`--refine`| If set, the script will: <br>&nbsp;&nbsp; - run phenix.real_space_refine. <br>&nbsp;&nbsp; - Then run phenix.mtriage and phenix.emringer to compute detailed metrics. <br><br>Use this if refinement is required before metric extraction.|
|`-n`| Limits the number of maps to process. <br>&nbsp;&nbsp; - If not provided: all maps selected in the selected category (`-m`) are processed. <br>&nbsp;&nbsp; - If set: only the first `n` entries from the list are processed.|
