...
In addition to the Python envornment, this workflow requires external software tools:

- **Phenix** (for map-model metrics computation).
- **MapQ** (for Q-score computation).
- Optional: post-processing methods for generating new maps (CryoTEN, EMReady, EMReady2, DeepEMhancerm LocScale, LocScale-, LocSpiral).

Detailed step-by-step installation instructions for all these components are provided in:
<pre>
